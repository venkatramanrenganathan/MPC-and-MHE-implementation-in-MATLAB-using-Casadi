...
rhs = [(v+velocityNoise)*cos(theta+thetaNoise)+systemNoise
       (v+velocityNoise)*sin(theta+thetaNoise)+systemNoise
       omega+systemNoise]; 
...
for iter = 1:montecarloIter
    ...
    st_hist(:,1, iter) = x0; % contains the history of states
    ...
    % NMPC simulaton loop: Loop until error > 10^-2 & #. MPC steps < max value.
    main_loop = tic;
    while(norm((x0-x_ref),2) > 1e-2 && mpciter < sim_time / T)
        ...
        [t0, x0, u0] = shift_with_noise(T, t0, x0, u,f);    
        % Update the state history
        st_hist(:,mpciter+2, iter) = x0;
...
figure(1); 
for k = 1:montecarloIter
    xH = [];
    yH = [];
    for kk = 1:size(st_hist,2)
        xH = [xH st_hist(1,kk,k)];
        yH = [yH st_hist(2,kk,k)];        
    end
    plot(xH, yH,'linewidth',1.5);
    hold on;    
end
for kk = 1:size(st_hist,2)
    aa = vec(st_hist(:,kk,:));
    aa = reshape(aa, 3,montecarloIter);
    aa(3,:) = [];
    aa      = aa';
    means{kk} = mean(aa);
    covar{kk} = cov(aa);
...
plot_distribution_movement(means,covar, size(st_hist,2))
